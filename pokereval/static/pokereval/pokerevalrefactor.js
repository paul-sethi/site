...
function groupSuitedHandsByRow(currentRow) {
	...
	for (var currentCol = currentRow + 1; currentCol < cards.length; currentCol++)
	{
		...
		{
			//TODO: create hand range from first and last	
			currentHandGroup = [];
		}
	}
}
...
function getSuitedHand(row, column) {
	var handText = cards[row] + cards[column] + "s";
    var hand = handDictionary[handText];
    return hand;
}

//var isConsecutiveHandGroup = false;
//	var consecutiveHandGroupStart = 0;
//	var consecutiveHandGroupEnd = 0;
//getNextNonConsecutiveHand
//getNextConsecutiveHand
//startConsecutiveHandGroup
//endConsecutiveHandGroup


//new HandGrid object
//numRows/numColumns?? seems unecessary
